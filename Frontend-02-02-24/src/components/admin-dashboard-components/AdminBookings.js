...
import { Deletebooking, fetchAllbookings } from '../../store/actions/bookingActions';
                  ...
                  <tbody>
                    {booking.map((booking) => (
                      <tr style={{ boxShadow: '0px 0px 1px rgba(0, 0, 0, 0.1), 0 0px 1px 0 rgba(0, 0, 0, 0.1)', borderRadius: "8px" }} key={booking._id}>
                        ...
                        <td className='td'>{booking?.Scheduled_Date ? booking.Scheduled_Date.slice(0, 10) : null}</td>
                        <td className='td'>
                          From: {booking.Time_Slot?.Start_time ? booking?.Time_Slot?.Start_time.slice(11, 16) : null}<br />
                          To: {booking.Time_Slot?.End_time ? booking?.Time_Slot?.End_time.slice(11, 16) : null}<br />
                        </td>
                        <td>
                          <button className='mx-1' onClick={() => EditBooking(booking._id)} style={{ border: 'none', backgroundColor: 'transparent' }}><i class="bi bi-pen-fill"></i></button>
