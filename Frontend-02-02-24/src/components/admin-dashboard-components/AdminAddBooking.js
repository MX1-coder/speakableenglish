...
import { fetchAllmeetings } from '../../store/actions/meetingActions';
...
const AdminAddBooking = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
      ...
      if (!note_for_Teacher || !student_ID || !teacher_ID || !package_ID || !scheduledDate || !timeSlot || !status || !meeting_ID) {
        // Find the name of the empty field
        const emptyFieldName = !note_for_Teacher ? 'Note for Teacher' :
          !student_ID ? 'Student' :
          !teacher_ID ? 'Teacher' :
          !package_ID ? 'Package' :
          !scheduledDate ? 'Scheduled Date' :
          !timeSlot ? 'Time Slot' :
          !status ? 'Status' : 'Meeting ID';
    ...
    <>
      ...
      <div className="form-group">
        ...
        <div className="form-group">
          ...
          <select value={package_ID} onChange={(e)=>setPackage_ID(e.target.value)}>
        ...
        <div className="form-group">
          ...
          <select value={timeSlot} onChange={(e) => { setTimeSlot(e.target.value) }}>
            ...
            {teacherbyID?.Availability?.map((availability) =>
              availability.Time?.map((timeSlotArray, index) => (
        ...
        <div className="form-group">
            <label htmlFor="Status">Select Meeting ID</label> &nbsp;&nbsp;
            <select
              name="meeting_ID"
              id='meeting_ID'
              value={meeting_ID}
