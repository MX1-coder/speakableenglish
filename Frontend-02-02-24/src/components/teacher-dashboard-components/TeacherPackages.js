    ...
    <div className='Package_mainPage_style'>
        ...
        {packages && packages.length > 0 ? (
          packages.map((pack, index) => (
            ...
            {pack.Course_IDs.map((val) => <img className='packagegimg mt-2' src={`http://localhost:3000/images/${val.Course_Images}`} alt=''/>)}
            <h6 className='mt-2'>{pack.Course_IDs.map((val) => val.Course_Name)}</h6>
