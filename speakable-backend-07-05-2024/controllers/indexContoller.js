...
exports.Signup_Student = async (req, res, next) => {
  try {
    ...
    const existingUser_Phone_Number = await Student.findOne({
      Phone_Number: Phone_Number,
    });
    if (existingUser_Phone_Number) {
      return res.status(409).json({
        message: "Student Phone_Number already exists",
      });
    }
...
const updateCoursesAssign = async (teacher, newCoursesAssign) => {
  const currentCoursesAssign = teacher.Courses_assign;

  const coursesToRemove = currentCoursesAssign.filter(
    (courseId) => !newCoursesAssign.includes(courseId)
  );
  const coursesToAdd = newCoursesAssign.filter(
    (courseId) => !currentCoursesAssign.includes(courseId)
  );

  // Remove teacher from courses that are no longer assigned
  const removePromises = coursesToRemove.map(async (courseId) => {
    try {
      const course = await Courses.findById(courseId);
      if (course) {
        course.Teachers_Details = course.Teachers_Details.filter(
  ...
  const addPromises = coursesToAdd.map(async (courseId) => {
    try {
      const course = await Courses.findById(courseId);
      if (course) {
        if (!course.Teachers_Details.includes(teacher._id)) {
          ...
          console.log(
            `Teacher ${teacher._id} is already assigned to course ${course._id}.`
          );
        }
      }
    } catch (error) {
...
exports.Delete_Enquiry_Student = async (req, res, next) => {
  try {
    ...
    // Find the student
    const DeletedEnquiry = await Enquiry_Student.findByIdAndDelete(Enquiry_ID);
    console.log(DeletedEnquiry);
    return res.json({
      message: "Enquiry student deleted successfully",
      DeletedEnquiry,
    });
  } catch (error) {
    next(error);
...
exports.Delete_Course = async (req, res, next) => {
  try {
    ...
    console.log(teacherIDsInCourse);
    await Promise.all(
    ...
    return res.json({ message: "Course deleted successfully", CourseDeleted });
  } catch (error) {
    ...
    await Promise.all(
      updatedTeacherDetails.map(async (teacherDetails) => {
        try {
          ...
          if (teacher) {
            // Check if CourseID already exists in Courses_assign
            if (!teacher.Courses_assign.includes(CourseID)) {
...
exports.Update_Booking = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Booking not updated successfully" });
  }
};
    ...
    imagekit.upload(
      ...
      function (error, result) {
        if (error) {
          ...
          return res.status(200).json({ filename }); // Send back the URL of the uploaded image
        }
      }
    );
  } catch (error) {
...
exports.Make_Payment = catchAsyncErrors(async (req, res, next) => {
  try {
      ...
      products: [
        {
          ...
          unitPrice: req.body.totalAmount * 100,
          quantity: "1",
        },
      ],
    };
    ...
    res
      .status(500)
      .json({ error: "Sorry You cannot Make Payment with Zero Amount" });
  }
});
  ...
  async (req, res, next) => {
    try {
      ...
      try {
        ...
        console.log(error);
        console.log("there is problem to login in owncloud");
      }
...
exports.GetExistingTeacherAvailability = catchAsyncErrors(
  async (req, res, next) => {
    try {
      ...
      // console.log(booking);
      const ExistingTeachers = booking.Teacher_ID; // Assuming Teacher_ID is an array, if not, remove the array brackets
      ...
      for (const teacher of ExistingTeachers) {
        ...
        console.log(TeacherTimeSlots);

        const formatDateToISO = (dateStr) => {
          const date = new Date(dateStr);
          return date.toISOString().split("T")[0]; // "2024-06-13"
        };
        const spreadDataExistingTeacher = TeacherTimeSlots.reduce(
          (acc, current) => {
            for (const [dateKey, value] of Object.entries(current)) {
              console.log(dateKey, "Today ---->", today);
              const formattedDateKey = formatDateToISO(dateKey);
              if (moment(formattedDateKey).isSameOrAfter(today)) {
                // Filter dates
                if (dateKey in acc) {
                  acc[dateKey].push(...value); // Spread the value array
                } else {
  ...
  async (req, res, next) => {
    try {
      ...
      for (const teacher of ExistingTeachers) {
        ...
        console.log(spreadDataAllBookedSlots);
...
async function createBookingAndPayment(orderData) {
  return bookingMutex.runExclusive(async () => {
    if (orderData.data) {
      console.log(orderData.data.orders[0]);
        ...
        if (val.buyer) {
          ...
          if (student) {
            for (const order of orderData.data.orders) {
              ...
              const Purchase_Amount = order.payMethod.amount / 100;
