          ...
          {Array.isArray(filteredTeacher) && filteredTeacher.length > 0 ? (
            filteredTeacher.map((teacher) => (
              <div key={teacher?._id} className="teacher_card">
                ...
                >
                  {t("AdminTeachers.Profile")}
                </button>
              </div>
            ))
          ) : (
            <p> {t("AdminTeachers.footer")}</p> // or some other fallback UI
          )}
        </div>
      </div>
    </>
